...
static inline bool add_pa_sync_base_subgroup_bis_cb(const struct bt_bap_bass_subgroup_bis *bis,
						    void *user_data)
{
...
static inline bool add_pa_sync_base_subgroup_cb(const struct bt_bap_bass_subgroup *subgroup,
						void *user_data)
{
	...
	ret = bt_bap_bass_subgroup_foreach_bis(subgroup, add_pa_sync_base_subgroup_bis_cb,
					       subgroup_param);
	if (ret < 0) {
		return false;
	}
